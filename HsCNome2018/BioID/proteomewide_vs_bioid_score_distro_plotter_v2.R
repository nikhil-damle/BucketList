...
{
 #ransam<-read.table(paste("mean1000X_PxPSSM_topScorer_distro_frm_REST_o_HsProteome_IUme",i,sep=""),sep="\t")
 ransam<-read.table(paste("mean1000X_LxPSSM_topScorer_distro_frm_REST_o_HsProteome_IUme",i,sep=""),sep="\t")
 dr<-density(ransam$V6,bw="SJ")
 xr<-hist(ransam$V6,breaks=15)
 xr$counts=log10(xr$counts)
 print(paste("Dealing with IU >= ",i))
 print(summary(ransam$V6))

 #bioid<-read.table(paste("SnD_highest_scoring_uniq12mer_IUme",i,sep=""),sep="\t")
 bioid<-read.table(paste("SnD_highest_scoring_uniq7mer_IUme",i,sep=""),sep="\t")
 ...
 #plot(xr,ylim=c(0,2.5),xlim=c(-25,50),xlab="PxIxIT score",cex.axis=1.5,cex.lab=1.5,ylab="Log10 (Frequency)",cex=1.2)
 plot(xr,ylim=c(0,2.5),xlim=c(-25,50),xlab="LxVP score",cex.axis=1.5,cex.lab=1.5,ylab="Log10 (Frequency)",cex=1.2)
 par(new=T)
 plot(x5,ylim=c(0,2.5),xlim=c(-25,50),xlab="",cex.axis=1.5,cex.lab=1.5,ylab="",col=rgb(1,0,0,0.3),cex=1.2)
 abline(v=mean(ransam$V6),lty=2,lwd=1.5,col="black")
 abline(v=mean(bioid$V3),col="red",lty=2,lwd=1.5)
 ...
 #(t.test(ransam$V6,bioid$V3,alternative="two.sided", var.equal=TRUE)) # alt H0 => diff in means ne 0
 print(t.test(bioid$V3,ransam$V6,alternative="greater", var.equal=TRUE)) # alt H0 => x > y
}
